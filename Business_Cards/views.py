...
from Business_Cards.models import edge_painted_business_cards_price, Extra_features, foil_business_cards_price, \
    raised_spot_uv_business_cards_price, raised_ink_business_cards_price, pantone_business_cards_price, \
    plastic_business_cards_price
...
def BC_Detail(request):
    ...
    urls_bc_aside = ["business-card-detail", "edge-painted-detail", "foil-business-card-detail",
                     "raised-spot-uv-business-card-detail", "pantone-business-cards-detail",
                     "plastic-business-cards-detail", "raised-ink-business-cards-detail"]
    ...
    urls_mp_aside = ["/marketing-products/calenders-detail", "/marketing-products/brouchers-flyers-detail",
                     "/marketing-products/postcards-detail", "/marketing-products/hangtags-detail",
                     "/marketing-products/labels-and-stickers-detail", "/marketing-products/ncr-forms-detail",
                     "/marketing-products/presentation-folder-detail", "custom-holiday-cards-detail"]

    b_urls = ["/boxes/pillow-boxes-detail", "/boxes/gable-boxes-detail", "/boxes/window-boxes-detail",
              "/boxes/mailer-boxes-detail", "/boxes/kraft-boxes-detail", "/boxes/cosmetics-boxes-detail",
              "/boxes/sleeve-boxes-detail", "/boxes/display-boxes-detail", "/boxes/beverage-boxes-detail",
              "/boxes/candle-boxes-detail", "/boxes/auto-parts-boxes-detail", "/boxes/pizza-boxes-detail"]
    ...
    if request.POST:
        ...
        total_price = ((float(price_paper) * float(quantity)) + (float(price_side) * float(quantity)) + (
                    float(price_size) * float(quantity)) + price_type) - price_discount
...
def Edge_painted_Detail(request):
    ...
    if request.POST:
        ...
        if size == 'US_Standard_Size':
            ...
            type_quantity_query = edge_painted_business_cards_price.objects.filter(quantity=quantity).values('id',
                                                                                                             'US_Standard_Size')
        ...
        total_price = ((float(price_paper) * float(quantity)) + (
                    float(price_side) * float(quantity)) + price_size_quantity)
...
def Plastic_business_card(request):
    ...
    if request.POST:
        ...
        # paper type price query
        # paper_price_query = Extra_features.objects.raw('SELECT id, paper_type_price FROM Business_Cards_Extra_features WHERE paper_type = %s', [paper_type])
        plastic_price_query = Extra_features.objects.filter(plastic_type=plastic_type).values('id',
                                                                                              'plastic_type_price')
...
def raised_ink_business_cards(request):
    ...
    if request.POST:
        ...
        if size == 'US_Standard_Size':
            extra_size = '2"x3.5" - US Standard Size'
            type_quantity_query = raised_ink_business_cards_price.objects.filter(quantity=quantity).values('id',
                                                                                                           'US_Standard_Size')
